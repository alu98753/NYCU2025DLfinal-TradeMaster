...
num_features = 16   # 新的特徵數量 (例如 open, high, low, close, adjcp, zopen, ..., zd_30)
...
try:
    ...
    assert relation_matrix_np.shape == (num_stocks, num_stocks), \
...
try:
    all_stocks_data_np = np.load(stocks_data_path) # 預期 shape: [num_stocks, num_days, num_features]
    ...
    for i in range(batch_size):
        ...
        if start_idx + window_len > loaded_num_days:
            print(f"警告: 數據不足以為 batch item {i} 創建不重疊的窗口，將重複使用第一個窗口。")
            start_idx = 0 # 如果數據不夠，就重複使用第一個窗口
        
        # 切片: [num_stocks, window_len, num_features]
        single_input_window_np = all_stocks_data_np[:, start_idx : start_idx + window_len, :]
        inputs_list.append(torch.from_numpy(single_input_window_np).float())
    
    inputs_from_file = torch.stack(inputs_list, dim=0) # 將列表堆疊成 batch -> [batch_size, num_stocks, window_len, num_features]
...
try:
    with torch.no_grad(): # 在測試時不需要計算梯度
        output_scores = asu_model(inputs_from_file, fake_mask) # <--- 使用從文件加載的數據

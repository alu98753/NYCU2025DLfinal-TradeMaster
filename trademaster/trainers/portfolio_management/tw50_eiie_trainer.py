...
class PortfolioManagementEIIETrainer(Trainer):
    def __init__(self, **kwargs):
        ...
        self.transition_shapes = OrderedDict({
            'state': (self.buffer_size, self.num_envs,
                      ...
                      self.state_dim),
            'prev_action': (self.buffer_size, self.num_envs, self.action_dim + 1),
    ...
    def train_and_valid(self):
        ...
        # print(f"self.shape:{state.shape}\n {self.action_dim}, {self.time_steps},{self.state_dim}")
        if self.num_envs == 1:
        ...
        while True:
            ...
            print(f"[Epoch {epoch}] Actor Loss: {logging_tuple[0]:.6f}")
            torch.set_grad_enabled(False)
            prev_action = torch.zeros((1, self.action_dim + 1), device=self.device)
            # print(f"buffer items : {buffer_items.undone}")
            # undone_np = buffer_items.undone.cpu().numpy()
            # import pandas as pd
            # df_undone = pd.DataFrame(undone_np)
            # df_undone.to_csv(os.path.join(self.work_dir, f"buffer_undone_epoch_{epoch}.csv"), index=False)
            # if torch.mean(buffer_items.undone) < 1.0:
            if (buffer_items.undone != 1.0).any():
                ...
                while True: ##一直用self.valid_environment.step(action)輸出當下應得的reward以及下一個state

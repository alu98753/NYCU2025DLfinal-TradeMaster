...
def parse_args():
    ...
    parser.add_argument("--config", default=osp.join(ROOT, "configs", "portfolio_management", "portfolio_management_tw50_HCAR_HCAR_adam_mse.py"),
...
def main():
    ...
    wandb.define_metric("Validation/*", step_metric="agent_step") # Log all Validation metrics against epoch
    ...
    wandb.define_metric("HCAR_Actor/*", step_metric="agent_step") # For internal actor stats
    # Add other per-step metrics here if any, e.g., learning rates if logged per step
    wandb.define_metric("Learning_Rate/*", step_metric="agent_step") # Or "epoch" if logged per epoch
    wandb.define_metric("Action/*", step_metric="agent_step") # Or "epoch" if logged per epoch
    ...
    causal_graph_path = osp.join(ROOT, "data/portfolio_management/tw50/ASU_data/pc_causal_relation.npy") 
    
    try:
        relation_matrix_np = np.load(causal_graph_path)
        assert relation_matrix_np.shape == (num_stocks_for_actor, num_stocks_for_actor), \
        ...
        print(f"Falling back to identity matrix for supports for {num_stocks_for_actor} stocks.")

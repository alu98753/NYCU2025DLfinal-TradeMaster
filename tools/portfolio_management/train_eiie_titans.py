...
def main():
    ...
    if hasattr(train_environment, 'stock_dim'):
        stock_dim = train_environment.stock_dim # N
        print(f"有stock_dim:{stock_dim}")
    else:
        print("警告: 環境 'train_environment' 沒有 'stock_dim' 屬性，將使用 'action_dim' 作為 stock_dim (N)。")
        stock_dim = action_dim # N

    print("--- Debug: cfg.act before build_net ---")
    print(cfg.act)

    # 更新 Actor 配置，確保包含 stock_dim
    cfg.act.update(dict(
        input_dim=input_dim,
        stock_dim=stock_dim,  # <--- 添加 stock_dim
        time_steps=time_steps
    ))

    # 更新 Critic 配置，確保包含 stock_dim (action_dim 也保留，以防 Agent 內部邏輯需要)
    cfg.cri.update(dict(
        input_dim=input_dim,
        stock_dim=stock_dim,  # <--- 添加 stock_dim
